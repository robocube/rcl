...
namespace RCL.Core
{
  public class TcpReceiveBox
  {
    ...
    public readonly Dictionary<RCSymbolScalar, RCValue> m_replies = new Dictionary<RCSymbolScalar, RCValue> ();
    public readonly Dictionary<RCSymbolScalar, TcpCollector> m_waiters = new Dictionary<RCSymbolScalar, TcpCollector> ();
    ...
    public void Remove (RCSymbolScalar id, TcpCollector waiter)
    {
      ...
      //Console.Out.WriteLine ("Client waiting on {0}", new RCSymbol(new RCArray<RCSymbolScalar>(m_waiters.Keys)));
    }
  }
}