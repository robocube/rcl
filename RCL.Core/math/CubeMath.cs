...
namespace RCL.Core
{
  public class CubeMath
  {
          ...
          for (int i = 0; i < attributes.Length; ++i)
          {
            ...
            try
            {
              if (verb.Profile == Profile.Monadic) {
                ...
                RCActivator.OverloadKey key =
                  new RCActivator.OverloadKey (verb.Name, typeof (RCCube), null, rtype);
                ...
                RCActivator.OverloadKey key0 = new RCActivator.OverloadKey (verb.Name,
                                                                            typeof (RCCube),
                                                                            ltype,
                                                                            rtype);
                ...
                // type and the collection type -- what to do?...
                ...
                // than the type which is being referenced.
                // GetElementType () gives you the referenced
                // type which is what we want when constructing generic methods.
                ...
                // The parameter is a ref parameter which is actually a different type
                // than the type which is being referenced.
                // GetElementType () gives you the referenced type which is what
                // we want when constructing generic methods.
                Type ctype = parameters[0].ParameterType;
                Type ltype = ctype.GetGenericArguments ()[0];
                Type rtype = parameters[1].ParameterType;
                Type otype = method.ReturnType;
                // The stype is not a parameter to the operator so it is not included in
                // the key.
                Type latype = typeof (RCVector<>).MakeGenericType (ltype);
    ...
    public delegate RCValue CubeOpLeftScalar <L, R, O> (RCVector<L> left,
                                                        RCCube right,
                                                        ScalarOp<L, R, O> op);
    public delegate RCValue CubeOpRightScalar <L, R, O> (RCCube left,
                                                         RCVector<R> right,
                                                         ScalarOp<L, R, O> op);

    public delegate RCValue SeqCubeOpMonadic <S, R, O> (RCCube right, SeqScalarOp<S, R, O> op);
    public delegate RCValue SeqCubeOpDyadic <S, R, O> (RCLong left,
                                                       RCCube right,
                                                       SeqScalarOp<S, R, O> op);
    public delegate RCValue SeqBlockOpMonadic <S, R, O> (RCBlock right, SeqScalarOp<S, R, O> op);
    public delegate RCValue SeqBlockOpDyadic <S, R, O> (RCLong left,
                                                        RCBlock right,
                                                        SeqScalarOp<S, R, O> op);
    public delegate RCValue ConCubeOp <C, L, R, O> (RCVector<L> left,
                                                    RCCube right,
    ...
    public static RCValue DyadicOpRightScalar <L, R, O> (RCCube left,
                                                         RCVector<R> right,
                                                         ScalarOp<L, R, O> op)
        where O : IComparable
    {
    ...
    /// </summary>
    protected static void EvalColumnarSingle <L, R, O> (Timeline timeline,
                                                        RCArray<int> lindex,
                                                        RCArray<L> ldata,
                                                        RCArray<int> rindex,
                                                        RCArray<R> rdata,
                                                        ...
                                                        ScalarOp<L, R, O> op) where O : IComparable
    {
      ...
      // The indices work better in this case because both arrays are setup on the same
      // timeline.
      int lt = -1, rt = -1;
    ...
    protected static void EvalColumnarNone<L, R, O> (RCCube left,
                                                     RCCube right,
                                                     RCArray<int> index,
                                                     RCArray<O> data,
                                                     ScalarOp<L, R, O> op)
    ...
    protected static void EvalColumnarObject<L, R, O> (Timeline ltimeline,
                                                       Column<L> llast,
                                                       Timeline rtimeline,
                                                       Column<R> rlast,
                                                       Timeline otimeline,
    ...
    protected static void EvalColumnarMulti<L, R, O> (Timeline ltimeline,
                                                      RCArray<int> lindex,
                                                      RCArray<L> ldata,
                                                      Timeline rtimeline,
                                                      RCArray<int> rindex,
                                                      ...
                                                      ScalarOp<L, R, O> op) where O : IComparable
    {
      ...
      while (true)
      {
        ...
        bool hasRight = ri < rdata.Count;
        if (hasLeft) {
          lt = ltimeline.TimeAt (lindex[li]);
        }
        if (hasRight) {
          rt = rtimeline.TimeAt (rindex[ri]);
        }
        if (!(hasLeft || hasRight)) {
          break;
        }
        ...
        for (int i = 1; i < right.Count; ++i)
        {
          ...
          if (resultCount != count) {
            throw new Exception (string.Format (
                                   "Expected all columns to have count {0}, but column {1} had count {2}",
                                   resultCount,
                                   i,
                                   count));
          }
        ...
        where S : struct where O : struct
    {
      if (right.Count == 0) {
        ...
        for (int col = 0; col < right.Columns.Count; ++col)
        {
          ...
          Dictionary<RCSymbolScalar, SeqState<S, O>> results = new Dictionary<RCSymbolScalar,
                                                                              SeqState<S, O>> ();
          ...
          for (int i = 0; i < rdata.Count; ++i)
          {
            ...
            while (scalar != null)
            {
              ...
              // We should change this to make it possible to say how many levels you
              // want.
        ...
        for (int i = 0; i < rdata.Count; ++i)
        {
          ...
          if (timeCompare < 0) {
            throw new Exception (string.Format ("Timeline out of order: time:{0} < prevTime:{1}",
                                                time,
                                                prevTime));
          }
            ...
            // Should we make aggregate rows appear AFTER their components in the time
            // series case?
    ...
    public static RCValue InvokeSequential (RCClosure closure,
                                            string name,
                                            RCVectorBase left,
                                            RCCube right)
    {
    ...
    public void EvalDyadic (RCRunner runner, RCClosure closure, RCCube left, RCCube right)
    {
      ...
      else if (left.Count == 0 && right.Count > 0 && right.Axis.Symbol != null &&
               right.Axis.ColCount == 1) {
        runner.Yield (closure, right);
      }
      else if (left.Count == 0) {
        runner.Yield (closure, new RCCube ());
      }
      else if (right.Count == 0 && left.Count > 0 && left.Axis.Symbol != null &&
               left.Axis.ColCount == 1) {
    ...
    public void EvalDyadic (RCRunner runner, RCClosure closure, object left, RCCube right)
    {
      ...
      if (right.Cols == 0) {
        ...
        runner.Yield (
          closure,
          Invoke (closure, op.Name, (RCVectorBase) left, right));
      }
    }
    ...
    public void EvalMonadic (RCRunner runner, RCClosure closure, RCCube right)
    {
      ...
      if (right.Cols == 0) {
        ...
        runner.Yield (
          closure,
          InvokeMonadic (closure, op.Name, right));
      }
    }
    ...
    public void EvalAggregate (RCRunner runner, RCClosure closure, RCLong left, RCCube right)
    {
      ...
      if (right.Cols == 0) {
        ...
        runner.Yield (
          closure,
          InvokeSequential (closure, op.Name, (RCVectorBase) left, right));
      }
    }
    ...
    public void EvalAggregate (RCRunner runner, RCClosure closure, RCCube right)
    {
      ...
      if (right.Cols == 0) {
        ...
        runner.Yield (
          closure,
          InvokeSequential (closure, op.Name, right));
      }
    }
    ...
    public void EvalRightContextual (RCRunner runner, RCClosure closure, RCCube left, object right)
    {
      ...
      if (left.Cols == 0) {
        ...
        runner.Yield (
          closure,
          Invoke (closure, op.Name, (RCVectorBase) right, left));
      }
    }

    // Use a dedicated function for this because it has non-standard handling for the
    // empty cube.
    ...
    public void ConcurrencyOp (RCRunner runner, RCClosure closure, RCCube left, RCCube right)
    {
      ...
      RCSystem.Activator.Invoke (
        ...
        closure,
        op.Name,
        left.GetSimpleVector (0),
        right.GetSimpleVector (0));
    }
    ...
    public class PlugLongComparer : System.Collections.IComparer
    {
    ...
    [RCVerb ("unplug")]
    public void Unplug (RCRunner runner, RCClosure closure, RCLong left, RCCube cube)
    {
      if (left.Count != 1) {
        throw new Exception ("plug takes only one default value on the left");
      }
    ...
    [RCVerb ("unplug")]
    public void Unplug (RCRunner runner, RCClosure closure, RCDouble left, RCCube cube)
    {
      if (left.Count != 1) {
        throw new Exception ("plug takes only one default value on the left");
      }
    ...
    /// <summary>
    /// Given a long in the left argument, key will rearrange parts of the S column using
    /// the same behavior as the part operator.
    /// </summary>
    [RCVerb ("key")]
    ...
    protected RCCube Key (RCArray<RCSymbolScalar> key, RCCube cube)
    {
      if (key.Count != cube.Axis.Count) {
        throw new Exception (string.Format (
                               "New symbol column must have the same count as the old one. New count: {0}, old count: {1}",
                               key.Count,
                               cube.Axis.Count));
      }
    ...
    protected RCCube Bang (RCCube left,
                           RCCube right,
                           string colname,
                           bool insert,
                           bool dedup,
    ...
    public RCCube Bang (RCCube left,
                        RCBlock right,
                        bool insert,
                        bool dedup,
                        bool force,
                        bool keepIncrs)
    {
      // For operators having no left cube parameter, the left axis will be built to match
      // the first
      // cube in the block.
      RCCube result = left;
      try
      {
        ...
        if (result.Axis.Time != null) {
          ...
          for (int i = 0; i < right.Count; ++i)
          {
            // Here we need to check to see if the cubes are time cubes and use
            // MergeMultipleCubes
    ...
    protected RCCube BangSymCubes (RCCube left,
                                   RCCube right,
                                   string colname,
                                   bool insert,
                                   bool dedup,
                                   ...
                                   bool keepIncrs)
    {
      ...
      if (insert) {
        for (int i = 0; i < right.Axis.Count; ++i)
        {
          ...
          if (!existing.Contains (symbol)) {
            result.Write (symbol);
            // Commenting this check out assumes there would not be any dups in the right
            // cube.
      ...
      for (int col = 0; col < right.Cols; ++col)
      {
        ...
        if (rightOnly.Contains (name)) {
          this.CopyUnsharedColumnSmart (right,
                                        result,
                                        col,
                                        name,
                                        keepIncrs: keepIncrs,
    ...
    protected void CopyUnsharedColumn (RCCube source,
                                       RCCube result,
                                       int col,
                                       string name,
                                       bool force,
    ...
    protected void CopyUnsharedColumnSmart (RCCube source,
                                            RCCube result,
                                            int col,
                                            string name,
                                            bool keepIncrs,
    ...
    protected void MergeSharedColumn (RCCube left,
                                      RCCube right,
                                      RCCube result,
                                      int col,
                                      string name,
    ...
    public RCCube Bang (RCCube left, RCVectorBase right, string colname, bool force, bool keepIncrs)
    {
      ...
      if (col < 0) {
        if (right.Count == 1) {
          ...
          throw new Exception (string.Format (
                                 "Unable to align vector having count {0} with cube having count {1}",
                                 right.Count,
                                 left.Axis.Count));
        }
      }
        ...
        // I think we should be able to get rid of this now that we have checks for locked
        // cubes.
        RCCube result = new RCCube (new RCArray<string> ("S"));
        for (int i = 0; i < left.Axis.Count; ++i)
        {
    ...
    /// <summary>
    /// Removes the timeline from a given cube, converting timeline cols into regular
    /// columns.
    /// </summary>
    [RCVerb ("untl")]
    ...
    public void EvalRows (RCRunner runner, RCClosure closure, RCLong left, RCCube right)
    {
      ...
      try
      {
        ...
        ReadSpec spec = new ReadSpec (counter,
                                      RCSymbol.Wild,
                                      (int) left[0],
                                      (int) left[1] + 1,
                                      false);
    ...
    protected static RCCube MergeMultipleCubes (string[] names, RCCube[] cubes)
    {
      ...
      // These maps take each cube from sorted (or should it be unsorted?) to merged.
      // Accounting for
      // matching timestamps.
      ...
      Dictionary<int, Dictionary<string, int>> columnMaps = new Dictionary<int, Dictionary<string,
                                                                                           int>> ();
