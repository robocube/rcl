...
namespace RCL.Core {
  ...
  public class Willow {
    ...
    [RCVerb ("willow")]
    public virtual void EvalWillow (RCRunner runner, RCClosure closure, RCLong right) {
          ...
          while (conflicts.Count > 0) {
            ...
            if (!space.TryGetValue (c, out s)) {
              Console.Out.WriteLine (
                "Collision point {0} does not exist. Conflict already resolved.",
                c);
              continue;
            }
            int limit = 0;
    ...
    protected void Remove (long t,
                           Point @from,
                           Point m0,
                           Dictionary<Point, Point> space,
                           Dictionary<Point, Point> removed) {
    ...
    protected void Readd (long t,
                          Point m0,
                          Point m1,
                          Dictionary<Point, Point> space,
                          Dictionary<Point, Point> removed,
                          Queue<Point> conflicts) {
  ...
  public class TopGun {
    ...
    [RCVerb ("topgun")]
    public virtual void EvalTopGun (RCRunner runner, RCClosure closure, RCLong right) {
    ...
    protected void Force (long t, Random random, Dictionary<Point, Line> space, List<Line> events) {
      ...
      while (checklist.Count > 0) {
        ...
        if (mover != null) {
          ...
          // insert the current link in between the common point and the object that is
          // moving.
          // then recalculate all the new positions for the moved object.
          // All of the moved points must be added to the checklist.
          // We need to look at two sets, points previously occupied by this object,
          // points now
          // occupied by this object.
          // The points in the new set that are not in the old set are the collision
          // candidates.
        }
      }
      // check collisions
    ...
    public void Fix (Dictionary<Point, Line> space,
                     List<Line> events,
                     Queue<Line> checklist,
                     Line
                     mover,
                     Line parent,
                     int choice) {
    ...
    public void State (Dictionary<Point, Line> space,
                       List<Line> events,
                       RCCube result,
                       ref int b,
                       int end) {
  ...
  public class Generator {
    ...
    [RCVerb ("annhilate")]
    public virtual void Annhilate (RCRunner runner, RCClosure closure, RCLong right) {
      EvalGenerator (runner, closure, right, CollisionRule.Annhilate);
    }

    [RCVerb ("reverberate")]
    public virtual void Reverberate (RCRunner runner, RCClosure closure, RCLong right) {
      EvalGenerator (runner, closure, right, CollisionRule.Reverberate);
    }

    public virtual void EvalGenerator (RCRunner runner,
                                       RCClosure closure,
                                       RCLong right,
                                       CollisionRule rule) {
  ...
  public class Quadra {
    ...
    [RCVerb ("quadra")]
    public virtual void EvalQuadra (RCRunner runner, RCClosure closure, RCLong right)
    {
      int time, bar;
      if (right.Count == 1) {
        time = (int) right[0];
      ...
      while (t < time) {
        ...
        for (t = begin; t < end; ++t) {
          ...
          while (true) {
            if (point.D < 3) {
              // Any new location must acquire two points of distance before it can
              // branch.
              ++point.D;
              break;
            }
              ...
              // do not collide children until they are big enough to do something
              // interesting.
              // any point with size less than 4 does not really have a shape, not
              // something we can
              // move.
              else if (dx < 4) {
                ++point.X.D;
                ...
                // If the distance between an object and its parent is 1 and a quantum
                // arrives,
        ...
        // For each node we want to know 2 things, its distance, its parent, and the time
        // it was
        // created.
