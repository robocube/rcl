  ...
  public class Now
  {
    ...
    public void EvalNow (RCRunner runner, RCClosure closure, RCBlock right)
    {
      runner.Yield (closure,
                    new RCTime (new RCTimeScalar (DateTime.UtcNow.Ticks,
    ...
    /// <summary>
    /// Given a series of year/month pairs, return a series of days in the corresponding
    /// month.
    /// </summary>
    [RCVerb ("daysInMonth")]
    public void EvalDaysInMonth (RCRunner runner, RCClosure closure, RCLong right)
    {
      if (right.Count % 2 != 0) {
        throw new Exception (
                "daysInMonth accepts a series of year/month pairs. The years are required on each pair.");
      }
