...
namespace RCL.Kernel
{
  public class NumberToken : RCTokenType
  {
    ...
    public static int LengthOfNumber (string text, int start, bool allowDash)
    {
      ...
      int current = start;
      if (current < text.Length && text[current] == 'N' && text[current + 1] == 'a' &&
          text[current + 2] == 'N') {
        return 3;
      }
      if (current < text.Length && text[current] == '-' && allowDash) {
        ++current;
      }
      ...
      if (current < text.Length && (current > start && text[current] == '.')) {
        ++current;
      }
      while (current < text.Length && (text[current] >= '0' && text[current] <= '9')) ++current;
      ...
      if (current < text.Length && (current > start && IsIn (text[current], NumTypes))) {
        ++current;
      }
      int length = current - start;
      if (length == 1 && IsIn (text[current - 1], NONNUMCHARS)) {
        return -1;
      }
