  ...
  public class RunnerTest
  {
    ...
    public void TestForConflictingResults ()
    {
      ...
      RCL.Kernel.Assert.AreEqual ("{status:0 data:5}",
                                  RepString (runner,
                                             "first #r from eval {serve:{b:bot {<-try {<-eval {<-2 + 3}}} f1:fiber {r:wait $b <-$r} <-wait $f1} r:wait fiber {<-serve #}}"));
    }
    ...
    public void TestTryError ()
    {
      ...
      RCL.Kernel.Assert.AreEqual ("{status:1 data:[?\n    <<Assert,Failed: assert false>>\n  ?]}",
                                  runner.Rep (
                                    "#status #data from try {<-assert false}").ToString ());
    }
    ...
    public void TestExitFromShellRemoteEval ()
    {
      ...
      for (int i = 0; i < 5; ++i)
      {
        if (RCSystem.IsMono ()) {
          runner.Rep (
            "eval {p:startx \"mono rcl.exe --output=clean --nokeys\" :$p writex \"exit 0\" :waitx $p}");
        }
        else {
          runner.Rep (
            "eval {p:startx \"rcl.exe --output=clean --nokeys\" :$p writex \"exit 0\" :waitx $p}");
        }
      }
    }
    ...
    public void TestExecError ()
    {
      ...
      if (RCSystem.IsMono ()) {
        runner.Rep (
          "p:startx \"mono rcl.exe --output=clean --show=print --nokeys --program=exit.o2\"");
      }
      else {
    ...
    public void TestMultipleCustomArguments ()
    {
      ...
      if (RCSystem.IsMono ()) {
        runner.Rep (
          "p:startx \"mono rcl.exe --output=clean first_argument --nokeys --custom1=one second_argument --custom2\"");
      }
      else {
        runner.Rep (
          "p:startx \"rcl.exe --output=clean first_argument --nokeys --custom1=one second_argument --custom2\"");
      }
