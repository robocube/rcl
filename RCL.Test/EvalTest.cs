...
namespace RCL.Test
{
  ...
  public class EvalTest
  {
    ...
    {
      DoEvalTest (
        "{go:{<-\"The \" + $animal + \" goes \" + $sound} duck:eval {animal:\"duck\" sound:\"quack\" go:$go} pig:eval {animal:\"pig\" sound:\"oink\" go:$go} chicken:eval {animal:\"chicken\" sound:\"cluck\" go:$go} <-eval {:duck.go # :pig.go # :chicken.go #}}",
    ...
    {
      DoEvalTest (
        "{go:{<-\"The \" + $animal + \" goes \" + $sound} duck:{animal:\"duck\" sound:\"quack\"} pig:{animal:\"pig\" sound:\"oink\"} chicken:{animal:\"chicken\" sound:\"cluck\"} <-eval {:$duck eval $go :$pig eval $go :$chicken eval $go}}",
        "{:\"The duck goes quack\" :\"The pig goes oink\" :\"The chicken goes cluck\"}");
    }
    ...
    {
      DoEvalTest ("{<-$R where $R like \"foo*\"} apply \"foobar\" \"foobaz\" \"fazban\" \"farbat\"",
                  "\"foobar\" \"foobaz\"");
    }
    ...
    {
      DoEvalTest ("{<-$R where 1 == long 1 part $R} apply #x,1,foo #y,1,bar #z,2,baz",
                  "#x,1,foo #y,1,bar");
    }
    ...
    {
      DoEvalTest ("{<-$R where $R < 2017.08.26} apply 2017.08.24 2017.08.25 2017.08.26",
    ...
    {
      DoEvalTest ("{<-not $R} apply true false true false true false",
    ...
    {
      DoEvalTest ("{<-$R where (names $R) like \"foo*\"} apply {foobar:1 fazban:2 foobad:3}",
                  "{foobar:1 foobad:3}");
    }
...
// #else
    // [ExpectedException(ExceptionType=typeof(System.Exception), ExpectedMessage="Unable
    // to resolve
    // name s.x")]
// #endif
    [Test]
    ...
    {
      // DoEvalTest ("{build:eval {actions:module {varA:\"a\" A:{:print $R.inputA + \" -
      // result\"}
      // B:{:A eval {inputA:$varA}} define:$actions} <-#define switch $actions} <-build.B
      // {}}", "");
      // DoEvalTest ("{n:eval {m:module {x:1 f0:{:$R.y} f1:{:f0 eval {y:$x}} define:$m}
      // <-#define
      // switch $m} <-n.f1 {}}", "{:{y:1}}");
      DoEvalTest ("{n:eval {m:module {x:1 f0:{y:$R.y + 2} f1:{:f0 eval {y:$x}}} <-$m} <-n.f1 {}}",
                  "{:{y:3}}");
    }
    ...
    {
      DoEvalTest (
        "{f:{<-true switch {:true switch {:true switch {:true switch {:$L - $R}}}}} <-13 f 9}",
        "4");
    }
    ...
    {
      DoEvalTest (
        "{loop:{:#x write {i:++} <-($L < $R) switch {:($L + 1) loop $R :$L}} f:long {<-fiber {<-0 loop 9}} each 0 to 4 :#x dispatch 25 :#x dispatch 25 <-0}",
        "0");
    }
    ...
    {
      DoEvalTest ("{f:{t:[?[!$x!]?] <-true switch {:t {} :\"y\"}} x:\"x\" t:[?[!f {}!]?] <-t {}}",
                  "\"x\"");
    }
    ...
    {
      DoEvalTest ("{f:{x:\"x0\" t:{<-$x} <-true switch {:t {} :\"y\"}} t:{<-f {}} <-t {}}",
                  "\"x0\"");
    }
    ...
    {
      // This one matters; the variable defined in the lexical scope of t:{<-$x} is the
      // one used.
      DoEvalTest ("{f:{x:\"x0\" t:{<-$x} <-true switch {:t {} :\"y\"}} x:\"x1\" t:{<-f {}} <-t {}}",
                  "\"x0\"");
    }
    ...
    {
      DoEvalTest ("{<-R {}} each {a:{<-1 + 2} count:{<-3 + 4} b:{<-count {}}}",
                  "{a:3 count:7 b:0}");
    }
    ...
    {
      DoEvalTest (
        "{f:{s:stack {} :assert 20 > $s.depth :assert 0 == $s.fiber <-($L < $R) switch {:($L + 1) f $R :$L}} <-0 f 100}",
        "100");
    }
    ...
    {
      DoEvalTest (
        "{loop:{s:stack {} :assert $s.depth < 20 :assert $s.fiber > 0 <-($L < $R) switch {:($L + 1l) loop $R :$L}} s0:stack{} :assert $s0.fiber == 0 f:eval {<-fiber {<-0 loop 100}} s1:stack {} :assert $s1.fiber == 0 :assert ($f at 1) = 1 <-wait $f}",
        "100");
    }
    ...
    {
      DoEvalTest (
        "{loop:{s:stack {} :assert 1 = $s.fiber :#x write {i:++} <-loop $R + 1} f:fiber {<-loop 0} :#x dispatch 100 :kill $f :try {:wait $f} <-0}",
        "0");
    }
    ...
    {
      DoEvalTest ("{f:{<-($R in 0.0 1.0) switch {:$R :(f $R - 1.0) + f $R - 2.0}} <-f 10.0}",
                  "55.0");
    }
    ...
    {
      DoEvalTest ("{factorial:{<-($R > 1) switch {:$R * factorial $R - 1 :1}} <-factorial 5}",
                  "120");
    }
    ...
    {
      DoEvalTest (
        "{r:{line:#x write {i:++} <-($L < $R - 1) switch {:($L + 1) r $R :$line}} f:fiber {<-0 r 10} <-wait $f}",
        "10");
    }
    ...
    public void TestRecursionMutual ()
    {
      ...
      DoEvalTest (
        "{f:{<-($R == 0) switch {:1 :$R - m f $R - 1}} m:{<-($R == 0) switch {:0 :$R - f m $R - 1}} :assert (long $f each 0 to 10) = 1 1 2 2 3 3 4 5 5 6 6 :assert (long $m each 0 to 10) = 0 0 1 2 2 3 4 4 5 6 6 <-0}",
        "0");
    }
    ...
    {
      // Assert.Throws<RCException> (delegate () { DoEvalTest ("{f:{<-$R.x} <-f 11}",
      // "0"); });
    ...
    public void TestRGetMissingName ()
    {
      ...
      // DoTestException ("{f:{<-$R get \"x\"} <-f {}}", RCErrors.Name, "Unable to resolve
      // name x");
    ...
    {
      // DoTestException ("{f:{<-\"x\" get $R} <-f {}}", RCErrors.Name, "Unable to resolve
      // name x");
    ...
    {
      // DoTestException ("{f:{<-1 get $R} <-f {}}", RCErrors.Range, "Index 1 is out of
      // range");
    ...
    public void TestWriteChecksCount ()
    {
      ...
      {
        DoRawTest ("#x write {a:1 10 b:2 20 c:3 30}", "0");
      });
    }
    ...
    {
      // I added these tests because I found after adding bytes vectors they didn't work
      // correctly
      // With read and write.
      DoEvalTest (
        "{:#x write {a:\\x00 b:\\x01 c:\\x02} :assert (#x read 0) = [S|a b c #x \\x00 \\x01 \\x02] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:write [S|a b c #x \\x00 \\x01 \\x02] :assert (#x read 0) = [S|a b c #x \\x00 \\x01 \\x02] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest ("{:write [S|x #a 0] f:fiber {<-#a read 1} :sleep 100 :assert not done $f <-0}",
                  "0");
    }
    ...
    {
      DoEvalTest ("{:write [S|x y #a,0 0 10 #a,1 1 11 #a,0 1 -- #a,1 2 --] <-#a,* read 2}",
                  "[S|x y #a,0 1 10 #a,1 2 11]");
    }
    ...
    public void TestWriteLastSparser ()
    {
      ...
      // ]
      // DoEvalTest ("{:write [S|x y selected #items,i0 100d 75d true #items,i1 200d 300d
      // false
      // #items,i0 -- -- false #items,i2 589d 136d true] <-#items snap 0 -1l}", "");

      // DoEvalTest ("{:write [S|dockto #items,i0 \"\" #items,i1 \"\" #items,i0 --
      // #items,i2 --]
      // <-#items snap 0 -1l}", "");
      // DoEvalTest ("{:write [S|x #items,i0 100d #items,i1 200d #items,i0 -- #items,i2
      // 589d]
      // <-#items snap 0 -1l}", "");

      DoEvalTest ("{:write [S|x y #s,1 100 1 #s,2 200 2 #s,1 -- -- #s,3 300 --] <-#s,* snap 0 -1}",
                  "[S|x y #s,1 100 1 #s,2 200 2 #s,3 300 --]");
      // DoEvalTest ("{:write [S|x y #s,0l 100d 1d #s,1l 200d 2d #s,0l -- -- #s,2l 589d
      // --] <-#s
      // snap 0 -1l}", "");
      // DoEvalTest ("{:write [S|x dockto #items,i0 100d \"\" #items,i1 200d \"\"
      // #items,i0 -- --
      // #items,i2 589d --] <-#items snap 0 -1l}", "");
      // DoEvalTest ("{:write [S|x selected dockto #items,i0 100d true \"\" #items,i1 200d
      // false
      // \"\" #items,i0 -- false -- #items,i2 589d true --] <-#items snap 0 -1l}", "");
      // DoEvalTest ("{:write [S|x y selected dockto #items,i0 100d  75d true \"\"
      // #items,i1 200d
      // 300d false \"\" #items,i0 -- -- false -- #items,i2 589d 136d true --] <-#items
      // snap 0
      // -1l}", "");
      // DoEvalTest ("{:write [S|id x y selected dockto #items,i0 \"i0\" 100d  75d true
      // \"\"
      // #items,i1 \"i1\" 200d 300d false \"\" #items,i0 -- -- -- false -- #items,i2
      // \"i2\" 589d
      // 136d true --] <-#items snap 0 -1l}", "");
      // DoEvalTest ("{:write [S|id x y mx my selected dockto #items,i0 \"i0\" 100d  75d
      //  0d  0d
      // true \"\" #items,i1 \"i1\" 200d 300d 0d 0d false \"\" #items,i0 -- -- -- -- --
      // false --
      // #items,i2 \"i2\" 589d 136d 75d 75d true --] <-#items snap 0 -1l}", "");
      // DoEvalTest ("{:write [S|id x y mx my w h selected dockto #items,i0 \"i0\" 100d
      //  75d  0d  0d
      // 150d 150d true \"\" #items,i1 \"i1\" 200d 300d 0d 0d 150d 150d false \"\"
      // #items,i0 -- --
      // -- -- -- -- -- false -- #items,i2 \"i2\" 589d 136d 75d 75d 150d 150d true --]
      // <-#items snap
      // 0 -1l}", "");
      // DoEvalTest ("{:write [T|S|id x y mx my w h selected dockto 635104349707739700l
      // #items,i0
      // \"i0\" 100d  75d  0d  0d 150d 150d true \"\" 635104349707964650l #items,i1 \"i1\"
      // 200d 300d
      // 0d 0d 150d 150d false \"\" 635104349829843180l #items,i0 -- -- -- -- -- -- --
      // false --
      // 635104349829845000l #items,i2 \"i2\" 589d 136d 75d 75d 150d 150d true --]
      // <-#items snap 0
      // -1l}", "");
      // DoEvalTest ("{:write [G|T|S|id x y mx my w h selected dockto 0l
      // 635104349707739700l
      // #items,i0 \"i0\" 100d  75d  0d  0d 150d 150d true \"\" 1l 635104349707964650l
      // #items,i1
      // \"i1\" 200d 300d 0d 0d 150d 150d false \"\" 2l 635104349829843180l #items,i0 --
      // -- -- -- --
      // -- -- false -- 3l 635104349829845000l #items,i2 \"i2\" 589d 136d 75d 75d 150d
      // 150d true --]
      // <-#items snap 0 -1l}", "");

      // DoEvalTest ("{:write [S|x y #s,a 1l 2l #s,b 10l 20l #s,a -- 3l #s,c 100l 200l]
      // <-#s snap 0
      // -1l}", "[S|x y #s,a 0l -- #s,b -- 2l]");
      // DoEvalTest ("{:write [S|x y #s,a 1d 2d] :write [S|x y #s,b 10d 20d] :write [S|x y
      // #s,a --
      // 3d] :write [S|x y #s,c 100d 200d] <-#s snap 0 -1l}", "[S|x y #s,a 0l -- #s,b --
      // 2l]");
    }
    ...
    {
      DoEvalTest ("{:write [E|S|x 25 #s,a 456.7] <-#s,* read 0 0}",
                  "[G|E|S|x 0 25 #s,a 456.7]",
                  RCFormat.Default);
    }
    ...
    {
      DoEvalTest ("{:write {E:25 S:#s,a x:456.7} <-#s read 0 0}",
                  "[G|E|S|x 0 25 #s,a 456.7]",
                  RCFormat.Default);
    }
    ...
    {
      DoEvalTest (
        "{:#x write {a:1 b:2 c:3} :assert (#x read 0) = [S|a b c #x 1 2 3] :#x write {a:10 b:20 c:30} :assert (#x read 1) = [S|a b c #x 10 20 30] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#s,0 #s,1 write {x:1 10} :#s,0 #s,1 write {x:2 20} :assert (#s,0 read 0) = [S|x #s,0 1 #s,0 2] :assert (#s,1 read 0) = [S|x #s,1 10 #s,1 20] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#s,1 #s,2 #s,3 write {x:10 20 30} :#s,2 #s,3 #s,1 write {x:20 35 10} :assert (#s,1 #s,2 #s,3 read 0) = [S|x #s,1 10 #s,2 20 #s,3 30 #s,3 35] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:write [S|x #s,1 10 #s,2 20 #s,3 30] :write [S|x #s,2 20 #s,3 35 #s,1 10] :assert (#s,1 #s,2 #s,3 read 0) = [S|x #s,1 10l #s,2 20 #s,3 30 #s,3 35] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#a write {x:1 y:2} :#a write {y:2 z:3} :assert (trace #a) = [S|x y z #a 1 2 -- #a -- -- 3] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#a force {x:1 y:2} :#a force {y:2 z:3} :assert (trace #a) = [S|x y z #a 1 2 -- #a -- 2 3] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#a force {x:1 y:2} :#a force {y:2 z:3} :assert (1 0 trace #a) = [S|x y z #a -- 2 3] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:force [S|x y #a 1 2] :force [S|y z #a 2 3] :assert (1 0 trace #a) = [S|x y z #a -- 2 3] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:force [S|x y #a 1 2] :force [S|y z #a 2 3] :assert (trace #a) = [S|x y z #a 1 2 -- #a -- 2 3] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{reader:fiber {<-#x read 0} :#x #x write {a:1 10 b:2 20 c:3 30} :assert (wait $reader) = [S|a b c #x 1 2 3 #x 10 20 30] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{reader:fiber {<-#x,* read 0} :#x,0 #x,1 write {a:1 10 b:2 20 c:3 30} :assert (wait $reader) = [S|a b c #x,0 1 2 3 #x,1 10 20 30] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{reader:fiber {<-#x read 0 3} :#x #x write {a:1 10 b:2 20 c:3 30} :#x write {a:100 b:200 c:300} :assert (wait $reader) = [S|a b c #x 1 2 3 #x 10 20 30 #x 100 200 300] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:write [E|S|x 0 #a 1 1 #a 2 2 #a 3] :assert (#a dispatch 1) = [G|E|S|x 0 0 #a 1] :assert (#a dispatch 2) = [G|E|S|x 1 1 #a 2 2 2 #a 3] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{reader:fiber {<-#x dispatch 1} :#x #x write {a:1 10 b:2 20 c:3 30} :assert (wait $reader) = [S|a b c #x 1 2 3] <-0}",
        "0");
    }
    ...
    {
      // This test could pass by accident if the second dispatch doesn't happen until
      // after the
      // second write.
      // This could be fixed with a gate operator that would let you wait on a signal from
      // another
      // fiber.
      DoEvalTest (
        "{:#x write {a:1 b:2 c:3} r0:#x dispatch 1 r1:fiber {<-#x dispatch 1} :#x write {a:10 b:20 c:30} :assert (wait $r1) = [S|a b c #x 10 20 30] <-0}",
        "0");
    }
    ...
    public void TestWriteDispatchMultiSymbolSuspended ()
    {
      ...
      DoEvalTest (
        "{:#s,x write {a:0 b:1 c:2} reader:fiber {<-#s,x #s,y dispatch 1} :sleep 50 :#s,y write {a:10 b:20 c:30} :assert (wait $reader) = [S|a b c #s,x 0 1 2 #s,y 10 20 30] :#s,x write {a:1 b:2 c:3} reader:fiber {<-#s,x #s,y dispatch 1} :sleep 50 :#s,y write {a:11 b:21 c:31} :assert (wait $reader) = [S|a b c #s,x 1 2 3 #s,y 11 21 31] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{reader:fiber {<-#x,* dispatch 1} :#x,0 #x,1 write {a:1 10 b:2 20 c:3 30} :assert (wait $reader) = [S|a b c #x,0 1 2 3] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{reader:fiber {<-#x dispatch 1} :#x,0 #x,1 #x write {a:1 10 100 b:2 20 200 c:3 30 300} :assert (wait $reader) = [S|a b c #x 100 200 300] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{reader:fiber {<-#x dispatch 1} :#x,0 #x,1 write {a:1 10 b:2 20 c:3 30} :#x write {d:4} :assert (wait $reader) = [S|a b c d #x -- -- -- 4] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{reader:fiber {<-#p,* dispatch 3} :#p,s,a #p,s,b #p,s write {x:2 3 5 y:4 5 9} :assert (wait $reader) = [S|x y #p,s,a 2 4 #p,s,b 3 5 #p,s 5 9] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#x,0 #x,1 write {a:1 10} :assert (#x,* dispatch 1) = [S|a #x,0 1] :assert (#x,* dispatch 1) = [S|a #x,1 10] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:write [S|a #x,0 1] r0:fiber {<-eval {c0:#x,* dispatch 2 c1:#x,* dispatch 2}} :sleep 20 :write [S|a #x,1 2 #x,0 10 #x,1 20] out:wait $r0 :assert $out.c0 = [S|a #x,0 1 #x,1 2] :assert $out.c1 = [S|a #x,0 10 #x,1 20] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:write [S|a b #x,0 0 #const #x,1 0 #const #x,0 1 #const1 #x,1 1 -- #x,0 2 --] c:#x,* dispatch 2 <-count $c.b}",
        "2");
    }
    ...
    {
      DoEvalTest (
        "{f:fiber {<-#x dispatch 0} :sleep 20 :assert not done $f :#x #x #x write {i:++ ++ ++} :assert (wait $f) = [S|i #x 0 #x 1 #x 2] f1:fiber {<-#x #y dispatch 0} :assert not done $f1 :#x write {i:++} :assert (wait $f1) = [S|i #x 3] <-0}",
        "0");
    }
    ...
    {
      // When writing cubes to the timeline, axis lines were being created even for
      // complete dups,
      // and this was
      // interfering with the counts for dispatch.
      DoEvalTest (
        "{:write cube {S:#x #x a:10 10} :assert (#x dispatch 0) = [S|a #x 10] :assert (#x peek 0) = false <-0}",
        "0");
    }
    ...
    {
      DoEvalTest ("{:assert not #x peek 0 :#x #x #x write {i:++ ++ ++} :assert #x #y peek 0 <-0}",
                  "0");
    }
    ...
    {
      DoEvalTest (
        "{f0:fiber {<-#x throttle 0} :wait $f0 :#x write {i:++} f1:fiber {<-#x #y throttle 0} :sleep 20 :assert not done $f1 :#x dispatch 1 :wait $f1 <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{f:fiber {<-#x #y gawk 0} :assert not done $f :#x write {i:++} :assert (wait $f) = [S|i #x 0] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{f:fiber {<-#x gawk 0} :#x,y write {i:++} :#x write {i:++} :assert (wait $f) = [S|i #x 0] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest ("{:write [S|a b #x 1 -- #y 10 2] :assert (#x dispatch 1) = [S|a b #x 1 --] <-0}",
                  "0");
    }
    ...
    {
      DoEvalTest (
        "{:#x write {i:++} :assert (#x dispatch 1) = [S|i #x 0] :#x write {i:++} :assert (#x dispatch 1) = [S|i #x 1] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:write [S|x #s,a 0 #s,b 1 #s,c 2] :assert (#s,* read 0) = [S|x #s,a 0 #s,b 1 #s,c 2] :assert (#s,* read 1) = [S|x #s,b 1 #s,c 2] :assert (#s,* read 2) = [S|x #s,c 2] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest ("{:write [S|x #a 10 #c 30 #b 20 #a 11 #a 12 #b 21] <-#a #b last 0}",
                  "[S|x #a 12 #b 21]");
    }
    ...
    {
      // would be cool if we operators to get the state of a fiber, like suspended, done,
      // active,
      // etc...
    ...
    {
      DoEvalTest (
        "{:write [S|x #a 0 #a 1 #a 2] :fiber {:#a throttle 3 :#check write {order:#second}} :#check write {order:#first} :#a dispatch 1 <-#check read 0 2}",
        "[S|order #check #first #check #second]");
    }
    ...
    {
      DoEvalTest (
        "{:write [S|x #a,0 0] f:fiber {<-#a,0 throttle 1} :#a,* dispatch 1 :assert (wait $f) = 1 <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:write [S|x #a,b,0 0] f:fiber {<-#a,* throttle 1} :#a,b,* dispatch 1 :assert (wait $f) = 1 <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:write [S|x #a 0] :assert (#a peek 2) = false :write [S|x #a 1] :assert (#a peek 2) = true <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#x write {i:++} f:fiber {<-#x last 1} :sleep 20 :assert not done $f :#x write {i:++} :sleep 20 :assert done $f <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#x write {i:++} :assert (#x gawk 1) = [S|i #x 0] :assert (#x dispatch 1) = [S|i #x 0] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:assert (#x poll 0) = [] :#x #x #x write {i:++ ++ ++} :assert (#x poll 0) = [S|i #x 0 #x 1 #x 2] :assert (#x poll 1) = [S|i #x 1 #x 2] :assert (#x poll 2) = [S|i #x 2] :assert (#x poll 3) = [] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#x,y write {a:1 b:2} :#x,z write {c:3} :assert (#x,* snap 1) = [S|a b c #x,z -- -- 3] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest ("{:write [S|x #a 1] :write [S|x #a 2] :write [S|x #a 3] u:#a read 0 <-$u.G}",
                  "0 1 2");
    }
    ...
    {
      DoEvalTest ("{:write [S|x #a 0 #a 1] :clear #a :write [S|x #a 2] u:#a read 0 1 <-lines $u}",
                  "3");
    }
    ...
    {
      DoEvalTest (
        "{:assert (#x page 0 2) = [] :#x #x #x #x #x write {i:++ ++ ++ ++ ++} :assert (#x page 0 2) = [S|i #x 0 #x 1] :assert (#x page 1 2) = [S|i #x 2 #x 3] :assert (#x page 2 2) = [S|i #x 4] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:assert (#x page 0 -2) = [] :#x #x #x #x #x write {i:++ ++ ++ ++ ++} :assert (#x page 0 -2) = [S|i #x 3 #x 4] :assert (#x page 1 -2) = [S|i #x 1 #x 2] :assert (#x page 2 -2) = [S|i #x 0] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:assert (#s,* snap 0 -1) = [] :#s,a write {i:++} :assert (#s,* snap 0 -1) = [S|i #s,a 0] :#s,a #s,b write {i:++ ++} :assert (#s,* snap 0 -1) = [S|i #s,a 1 #s,b 0] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#s,a write {x:1} :#s,b write {x:10} :#s,c write {x:100} :#s,c write {x:100} :#s,d write {x:1000} :assert (#s,* snap 0 -1) = [S|x #s,a 1 #s,b 10 #s,c 100 #s,d 1000] <-0}",
        "0");
      // DoEvalTest ("{:#s,a write {x:1l y:10l z:100l} :#s,b write {x:2l y:20l z:200l}
      // :#s,c write
      // {x:3l y:30l z:300l} :#s,d write {x:4l y:40l z:400l} :#s,c write {x:3l y:30l
      // z:300l} :assert
      // (#s snap 0 -1l) = [S|x y z #s,a 1l 10l 100l #s,b 2l 20l 200l #s,c 3l 30l 300l
      // #s,d 4l 40l
      // 400l] <-0l}", "0l");
    }
    ...
    {
      DoEvalTest (
        "{:#s,a write {x:1} :#s,b write {x:10} :#s,a write {x:2} :assert (#s,* snap 0 -1) = [S|x #s,b 10 #s,a 2] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#s,a,0 write {x:1} :#s,b,0 write {x:10} :#s,a,0 write {x:2} :assert (#s,* snap 0 -1) = [S|x #s,b,0 10 #s,a,0 2] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest ("{:write [S|x y #s,a 1 -- #s,b -- 2] <-#s,* snap 0}",
                  "[S|x y #s,a 1 -- #s,b -- 2]");
    }
    ...
    {
      DoEvalTest ("{:write [S|x y #s,a 1 -- #s,b 2 -- #s,c -- 3] <-#s,* snap 0}",
                  "[S|x y #s,a 1 -- #s,b 2 -- #s,c -- 3]");
    }

    [Test]
    public void TestSnapSparseFirstRow2 ()
    {
      DoEvalTest ("{:write [S|x #s,a 0 #s,b 1] :write [S|y #s,c 2 #s,d 3] <-#s,* snap 0}",
                  "[S|x y #s,a 0 -- #s,b 1 -- #s,c -- 2 #s,d -- 3]");
    }
    ...
    {
      DoEvalTest (
        "{f:fiber {<-#a read 1} :#a write {x:0} :clear #a :#a write {x:1} :assert (wait $f) = [S|x #a 1] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{f:fiber {<-#a read 1} :write [S|x #a 0] :clear #a :write [S|x #a 1] :assert (wait $f) = [S|x #a 1] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:#a write {x:0} :clear #a :#a write {x:1} :assert (#a read 1) = [S|x #a 1] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{:write [S|x #a 0] :clear #a :write [S|x #a 1] :assert (#a read 1) = [S|x #a 1] <-0}",
        "0");
    }
    ...
    {
      DoEvalTest ("{:write [S|x #a 0 #a 1] <-#a read 0}",
                  "[G|E|S|x 0 0 #a 0 1 1 #a 1]",
                  RCFormat.Default);
    }
    ...
    {
      DoEvalTest (
        "{:write [S|x #page,a 1 #page,b 2 #page,c 3] f:fiber {<-#page,d,* read 0 1} :sleep 20 :write [S|x #page,d,x 4] :wait $f <-0}",
        "0");
    }
    ...
    public void TestSelect1 ()
    {
      ...
      DoEvalTest (string.Format ("#a select {0}", ustring),
                  "[G|S|x y z 0 #a 1 10 100]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#b select {0}", ustring),
                  "[G|S|x y z 1 #b 2 20 200]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#c select {0}", ustring),
                  "[G|S|x y z 2 #c 3 30 300]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#* select {0}", ustring),
                  "[G|S|x y z 0 #a 1 10 100 1 #b 2 20 200 2 #c 3 30 300]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#a #b select {0}", ustring),
                  "[G|S|x y z 0 #a 1 10 100 1 #b 2 20 200]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#c #b select {0}", ustring),
                  "[G|S|x y z 1 #b 2 20 200 2 #c 3 30 300]",
                  RCFormat.Default);
      ...
      DoEvalTest (string.Format ("#a,* select {0}", vstring),
                  "[G|S|x y z 0 #a,x 1 10 100 1 #a,y 2 20 200]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#b,* select {0}", vstring),
                  "[G|S|x y z 2 #b,z 3 30 300]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#b,z select {0}", vstring),
                  "[G|S|x y z 2 #b,z 3 30 300]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#a,x select {0}", vstring),
                  "[G|S|x y z 0 #a,x 1 10 100]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#a,x #b,z select {0}", vstring),
                  "[G|S|x y z 0 #a,x 1 10 100 2 #b,z 3 30 300]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#a,* #b,z select {0}", vstring),
                  "[G|S|x y z 0 #a,x 1 10 100 1 #a,y 2 20 200 2 #b,z 3 30 300]",
                  RCFormat.Default);
      ...
      DoEvalTest (string.Format ("#1,a select {0}", wstring),
                  "[G|S|x y z 0 #1,a 1 10 100]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#1,b select {0}", wstring),
                  "[G|S|x y z 1 #1,b 2 20 200]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#2,c select {0}", wstring),
                  "[G|S|x y z 2 #2,c 3 30 300]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#1,c select {0}", wstring), "[]", RCFormat.Default);
      DoEvalTest (string.Format ("#2,* select {0}", wstring),
                  "[G|S|x y z 2 #2,c 3 30 300]",
                  RCFormat.Default);
      ...
      DoEvalTest (string.Format ("#2,* select {0}", xstring),
                  "[G|S|x y z 2 #2,c,0 3 30 300]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#1,* select {0}", xstring),
                  "[G|S|x y z 0 #1,a,0 1 10 100 1 #1,b,0 2 20 200]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#1,a,* select {0}", xstring),
                  "[G|S|x y z 0 #1,a,0 1 10 100]",
                  RCFormat.Default);
      DoEvalTest (string.Format ("#2,b,* select {0}", xstring), "[]", RCFormat.Default);
      DoEvalTest (string.Format ("#1,b,* select {0}", xstring),
                  "[G|S|x y z 1 #1,b,0 2 20 200]",
                  RCFormat.Default);
    }
    ...
    {
      DoEvalTest ("#1,*,0 select [S|x y z #1,a,0 1 10 100 #1,b,0 2 20 200 #2,b,0 3 30 300]",
                  "[G|S|x y z 0 #1,a,0 1 10 100 1 #1,b,0 2 20 200]",
                  RCFormat.Default);
    }
    ...
    {
      DoEvalTest ("#*,*,0 select [S|x y z #1,a,0 1 10 100 #1,b,0 2 20 200 #2,b,0 3 30 300]",
                  "[G|S|x y z 0 #1,a,0 1 10 100 1 #1,b,0 2 20 200 2 #2,b,0 3 30 300]",
                  RCFormat.Default);
    }
    ...
    {
      DoEvalTest ("#*,b,* select [S|x y z #1,a,0 1 10 100 #1,b,0 2 20 200 #2,b,0 3 30 300]",
                  "[G|S|x y z 1 #1,b,0 2 20 200 2 #2,b,0 3 30 300]",
                  RCFormat.Default);
    }
    ...
    // {
    //  DoEvalTest ("#SPY,PUT,BUY #SPY,PUT,SELL select [S|qty # 70 #SPY 64 #SPY,PUT 64
    // #SPY,PUT,BUY
    // 32 #SPY,PUT,SELL 32]", "[S|qty #SPY,PUT,BUY 32 #SPY,PUT,SELL 32]",
    // RCFormat.Default);
    // }
    ...
    {
      DoEvalTest (
        "{b0:bot {:#x write {i:++} <-wait 0} b1:bot {:#y write {i:++} <-wait 0} :assert (first #x from dump $b0) = [S|i #x 0] :assert (first #y from dump $b1) = [S|i #y 0] <-0}",
        "0");
    }
    ...
    public void TestKillBot ()
    {
      ...
      // Let's just keep it that way until we have good tools for visualizing the state of
      // the
      // system.
      // Update Dec 2, 2014. This was one of the earliest tests for bot lifecycle stuff.
      // It used the blackboard log to tell when fibers were done. I'm getting rid of the
      // blackboard
      // log
      ...
      // DoEvalTest ("{f1:{<-f1 $R + 1l} f2:{<-f2 $R & 1l} b:bot {:fiber {<-f1 0l} :fiber
      // {<-f2 0l}
      // <-wait 0l} :(#log,fiber + (symbol $b) + #1l #2l) dispatch 1l :kill $b
      // <-(#log,fiber +
      // (symbol $b) + #1l #2l) dispatch 1l}", "[S|state #log,fiber,1l,1l \"killed\"
      // #log,fiber,1l,2l \"killed\"]");
      // I think we are going to need to convert RCNative into RCException and give it
      // some proper
      // syntax.
      DoEvalTest (
        "{f1:{<-f1 $R + 1} f2:{<-f2 $R & 1} b:bot {:fiber {<-f1 0} :fiber {<-f2 0} <-wait 0} :kill $b <-unwrap #status from try {<-wait $b}}",
        "1");
    }
    ...
    {
      DoEvalTest (
        "{html:[?<html><head>[!$R.head!]</head><body>[!$R.body!]</body></html>?] <-html {head:\"a head\" body:\"a body\"}}",
    ...
    {
      DoEvalTest (
        "{head:[?<head>a head</head>?] body:[?<body>a body</body>?] html:[?<html>[!head {}!][!body {}!]</html>?] <-html {}}",
        "\"<html><head>a head</head><body>a body</body></html>\"");
    }
    ...
    {
      DoEvalTest ("#testcanonical format parse \"[S|x #a 10.0 #a -- #a 30.0]\"",
                  "\"[S|x #a 10.0 #a -- #a 30.0]\"");
    }
    ...
    {
      DoEvalTest (
        "#testcanonical format parse \"[S|bp ap #x 1001 1003 #y 2002 -- #x 1002 -- #x 1001 1002]\"",
        "\"[S|bp ap #x 1001 1003 #y 2002 -- #x 1002 -- #x 1001 1002]\"");
    }
    ...
    {
      // Once we get some traction with demo/messaging, should be able to deprecate this
      // test and
      // the operators it depends on.
      DoEvalTest (
        "{http:httpstart \"http://*:8000/\" server:fiber {context:httprecv $http qs:httpqs $context <-$context httpsend eval {i:$qs.i+1}} request:{<-($R.i < 100) switch {:request \"http://localhost:8000/\" httpget $R :$R}} output:request {i:0} :httpstop $http <-$output}",
        "{i:100}");
    }
    ...
    public void TestHttpSendMulti ()
    {
      ...
      // This test sometimes get stuck when run from inside the runner, but never under
      // the normal
      // shell.
      // I have convinced myself that it's something to do with monodevelop ide and not my
      // code.
      // So I'm ignoring it for now until we create some superior test harness for working
      // with
      // distributed apps.
      // eval {serve:{r1:httprecv $h r2:httprecv $h :($r1 & $r2) httpsend "{}" <-0l}
      // h:httpstart
      // "http://*:8001/test/" s:fiber {<-serve $h} c1:fiber
      // {<-"http://localhost:8001/test" httpget
      // {}} c2:fiber {<-"http://localhost:8001/test" httpget {}} :wait $c1 :wait $c2
      // :wait $s
      // :httpstop $h <-0l}
      DoEvalTest (
        "{serve:{r1:httprecv $h r2:httprecv $h :($r1 & $r2) httpsend \"{}\" <-0} h:httpstart \"http://*:8001/test/\" s:fiber {<-serve $h} c1:fiber {<-\"http://localhost:8001/test\" httpget {}} c2:fiber {<-\"http://localhost:8001/test\" httpget {}} :wait $c1 :wait $c2 :wait $s :httpstop $h <-0}",
        "0");
    }
    ...
    {
      DoEvalTest (
        "{status:unwrap #status from try {:\"exit.rcl\" save #pretty format {go:exit 21} exe:(codebase {}) + \"/rcl.exe\" <-exec \"mono --debug \" + $exe + \" --program=exit.rcl --action=go\"} :assert $status = 21 <-0}",
        "0");
    }
#endif
